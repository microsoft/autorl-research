...
for epoch in epoch_ranges:
    ...
    rollout_states = info.pop("rollout_states")
    info = pd.DataFrame(info)
    info.index.name = "Timestep"
    all_info[(args.seed, epoch)] = info
    
    # rollout_states = np.stack(rollout_states, axis=0)
    # images = renderer.render_observations(rollout_states)
    # images_savepath = os.path.join(args.output_dir, f"epoch{epoch}_result.gif")
    # renderer.save_gif(images, images_savepath)
