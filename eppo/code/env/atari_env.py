...
cls_dir_dict = {
    "AlienNoFrameskip-v4": "code/env/alien.pkl",
    "AmidarNoFrameskip-v4": "code/env/amidar.pkl",
    "AssaultNoFrameskip-v4": "code/env/assault.pkl",
    "BreakoutNoFrameskip-v4": "code/env/breakout.pkl",
    "PongNoFrameskip-v4": "code/env/pong.pkl",
    "SeaquestNoFrameskip-v4": "code/env/seaquest.pkl",
}
...
class DNCWrapper(gym.Wrapper):
    def __init__(self, env, env_name):
        super().__init__(env)
        self.index=0
        self.loaded_trans = pkl.load(open(cls_dir_dict[env_name],"rb"))
    
    def reset(self):
        obs = self.env.reset()
        self.index = self.loaded_trans.predict([obs.flatten()])[0]
        return obs
...
def wrap_deepmind(env_id, episode_life=True, clip_rewards=True,scale=False,frame_stack=True, dnc=False, env_name="PongNoFrameskip-v4"):
    ...
    if dnc:
        env = DNCWrapper(env, env_name)
    return env


if __name__=="__main__":
    # env = MySodobanEnv(num_envs=200)
    # env.collect_init_state()

    env_list = ["AlienNoFrameskip-v4","AmidarNoFrameskip-v4","AssaultNoFrameskip-v4","BreakoutNoFrameskip-v4","PongNoFrameskip-v4","SeaquestNoFrameskip-v4"]
    for env_name in env_list:
        env = wrap_deepmind(env_name)
        env.seed(99)
        data = []
        for i in range(2000):
            obs = env.reset()
            obs = obs.flatten()
            data.append(obs)
        pkl.dump(data,open(f'./code/env/{env_name}_init.pkl',"wb"))
